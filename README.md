...
- **Payments** — pay for orders directly from the order detail page
...
- **Multiple images** — product listings support multiple image URLs
...
- **Polish language UI** — interface fully in Polish (Zamówienia, Produkty, Płatność, etc.)
...
│       └── utils/             # JWT helpers, cookie utilities
│
└── frontend/
    ├── package.json
    ├── tsconfig.json
    ...
    └── src/
        ...
        ├── api/               # Axios instance & API functions (auth, products, cart, orders, admin, etc.)
        ...
        ├── hooks/             # Custom hooks (useAuth, useProducts, useOrders, etc.)
        ├── types/             # TypeScript type definitions
        └── utils/             # formatPrice, orderStatus helpers, etc.
```
...
| Auth | `/login`, `/register` | Guests only (redirects if authenticated) |
