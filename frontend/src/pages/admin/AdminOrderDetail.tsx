...
import { useAdminOrder, useAdminUpdateOrderStatus } from '@/hooks/useOrders';
...
const AdminOrderDetail = () => {
  ...
  const handleStatusChange = (status: OrderStatus) => {
    if (window.confirm(`Zmienić status zamówienia na "${orderStatusLabels[status]}"?`)) {
    ...
    <div className="space-y-8">
      ...
      <div className="overflow-hidden rounded-3xl border border-brand-200 bg-white shadow-md">
        <div className="bg-brand-800 px-6 py-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-brand-300">Panel administratora</p>
        ...
        <div className="px-6 py-4">
          <dl className="divide-y divide-brand-50 rounded-xl border border-brand-100 bg-cream-50">
            ...
            <div className="flex items-center justify-between px-4 py-2.5 text-sm">
              ...
              <dd className="text-base font-extrabold text-brand-800">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="overflow-hidden rounded-2xl border border-brand-100 bg-white shadow-sm">
          <div className="flex items-center gap-3 border-b border-brand-100 px-6 py-4">
            ...
            <h2 className="text-lg font-semibold text-brand-800">Kupujący</h2>
          </div>
          <div className="px-6 py-4">
            <dl className="divide-y divide-brand-50 rounded-xl border border-brand-100 bg-cream-50">
              <div className="flex items-center justify-between px-4 py-2.5 text-sm">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 text-sm">
                ...
                <dd className="font-medium text-brand-800">
                  <a href={`mailto:${order.buyer.email}`} className="hover:underline">{order.buyer.email}</a>
                </dd>
              </div>
              <div className="flex items-center justify-between px-4 py-2.5 text-sm">
                <dt className="text-gray-500">ID</dt>
                <dd className="font-mono text-xs text-brand-600">{order.buyer.id}</dd>
              </div>
            </dl>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-100 bg-white shadow-sm">
          <div className="flex items-center gap-3 border-b border-brand-100 px-6 py-4">
            ...
            <h2 className="text-lg font-semibold text-brand-800">Sprzedający</h2>
          </div>
          <div className="px-6 py-4">
            {order.seller ? (
              <dl className="divide-y divide-brand-50 rounded-xl border border-brand-100 bg-cream-50">
                ...
                <div className="flex items-center justify-between px-4 py-2.5 text-sm">
                  <dt className="text-gray-500">E-mail</dt>
                  <dd className="font-medium text-brand-800">
                    <a href={`mailto:${order.seller.email}`} className="hover:underline">{order.seller.email}</a>
                  </dd>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5 text-sm">
                  <dt className="text-gray-500">ID</dt>
                  <dd className="font-mono text-xs text-brand-600">{order.seller.id}</dd>
                </div>
              </dl>
            ) : (
              <p className="text-sm italic text-gray-400">Brak przypisanego sprzedającego (zamówienie multi-seller).</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-brand-100 bg-white shadow-sm">
            ...
            <div key={product.id} className="flex items-center gap-4 py-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-sm text-gray-500">
                  {product.OrderItem.quantity} szt. x {formatPrice(parseFloat(product.OrderItem.price_per_unit.toString()))} / szt.
                </p>
        ...
        <div className="flex items-center justify-between border-t border-brand-100 px-6 py-4">
          <span className="text-base font-semibold text-brand-800">Łącznie</span>
      ...
      <div className="overflow-hidden rounded-2xl border border-brand-100 bg-white shadow-sm">
        <div className="flex items-center gap-3 border-b border-brand-100 px-6 py-4">
          ...
          <h2 className="text-lg font-semibold text-brand-800">Zmień status zamówienia</h2>
        </div>
