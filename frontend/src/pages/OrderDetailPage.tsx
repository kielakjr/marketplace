...
const OrderDetailPage = () => {
  ...
  const submitRating = useSubmitRating(order?.seller_id ?? '');

  const handleSubmitRating = () => {
    if (!ratingValue || !order) return;
    submitRating.mutate(
      { orderId: order.id, rating: ratingValue, comment: comment || undefined },
  ...
  return (
    <div className="space-y-8">
      ...
      {order.status === 'COMPLETED' && order.seller_id && (
        <div className="rounded-2xl border border-brand-100 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 border-b border-brand-100 px-6 py-4">
            ...
            <h2 className="text-lg font-semibold text-brand-800">Oceń sprzedawcę</h2>
          </div>
          ...
          <div className="p-6">
            {isRated ? (
              <div className="flex items-center gap-2 text-sm font-medium text-green-700">
                ...
                Dziękujemy za wystawienie oceny!
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-brand-600 mb-2">Twoja ocena</p>
                  <StarRating value={ratingValue} onChange={setRatingValue} />
                </div>
                <div>
                  <label className="text-sm text-brand-600 block mb-1">Komentarz (opcjonalnie)</label>
                  <textarea
                    value={comment}
                    onChange={(e) => setComment(e.target.value)}
                    rows={3}
                    placeholder="Napisz komentarz..."
                    className="w-full rounded-xl border border-brand-200 bg-cream-50 px-4 py-2.5 text-sm text-brand-800 placeholder-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none"
                  />
                </div>
                <Button
                ...
                {submitRating.isError && (
                  <p className="text-sm text-red-600">
                    Nie udało się wysłać oceny. Możliwe, że już oceniłeś to zamówienie.
                  </p>
                )}
            </div>
        )}
